...
do i=1,om_mesh
call self_energy(layer_2,k_pt(k_inp,:),omeg(i),size(layer_2),nn,size_hp,self_l(i,:,:),self_r(i,:,:))
enddo
...
do i=1,num_avg
    ...
    do j=1,om_mesh
call setup_surface(k_pt(k_inp,:),layer_3,size(layer_3),nn,omeg(j),self_l(j,:,:),self_r(j,:,:),transmit)
        transmission(j) = transmission(j) + real(transmit)
        print*,real(omeg(j)),transmission(j)
    enddo
    deallocate(layer_3)
enddo
...
do i=1,om_mesh
    write(2333,*)real(omeg(i)),transmission(i)
enddo
close(2333)
